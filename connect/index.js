...
async function startR(){
    ...
    xcaa.sendMessage(anu.id, {audio: butff, ptt:true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", "forwardingScore": 1000000000, isForwarded: true, sendEphemeral: true, externalAdReply: {"title": teks1, "body": `${ddate}`, "previewType": "PHOTO", "thumbnailUrl": bufft, "thumbnail": bufft, "sourceUrl": "https://wa.me/6283818221226"}}})
    } else if (anu.action == 'remove') {
    let lvpantex = fs.readFileSync('./media/leavee.mp3')
    let lvbuft = await getBuffer(ppuser)
    teks2 = `Sayonara @${num.split("@")[0]} 👋`
    xcaa.sendMessage(anu.id, {audio: lvpantex, ptt:true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", "forwardingScore": 1000000000, isForwarded: true, sendEphemeral: true, externalAdReply: {"title": teks2, "body": `Pantek`, "previewType": "PHOTO", "thumbnailUrl": lvbuft, "thumbnail": lvbuft, "sourceUrl": "https://wa.me/6283818221226"}}})
    }
    }
    } catch (err) {
      console.log(err)
