...
async function startR(){
    ...
    xcaa.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: tekswell })
    ...
    xcaa.reSize = async (image, width, height) => {
    ...
    // Send Button 5 Lokasi
    xcaa.send5ButLoc = async (jid , text = '' , footer = '', lok, but = [], options = {}) =>{
    let resize = await xcaa.reSize(lok, 300, 150)
    ...
    xcaa.sendButtonMsg = (jid, text = '' , footer = '', but = []) =>{
