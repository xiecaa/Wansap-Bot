...
async function startR(){
    ...
    xcaa.sendMessage(anu.id, {audio: butff, ptt:true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", forwardingScore: 999999, isForwarded: true, externalAdReply: {title: teks1, body: "", mediaType: "2", thumbnail: bufft, mediaUrl: 'https://youtu.be/TaG15W_8f_s'}}})
    } else if (anu.action == 'remove') {
    let lvpantex = fs.readFileSync('./media/leavee.mp3')
    let lvbuft = await getBuffer(ppuser)
    teks2 = `Sayonara @${num.split("@")[0]} 👋`
    xcaa.sendMessage(anu.id, {audio: lvpantex, ptt:true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", forwardingScore: 999999, isForwarded: true, externalAdReply: {title: teks2, body: `${ddate}`, mediaType: "2", thumbnail: lvbuft, mediaUrl: global.linkgc}}})
    }
    }
    } catch (err) {
      console.log(err)
