...
*/
require('../settings')
const { default: makeWASocket, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, AnyMessageContent, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startR(){
    ...
    cfonts.say('Author     : FxSx', {font: 'console', gradient: ['red','magenta'], align: 'center'})
    cfonts.say('Library     : Baileys', {font: 'console', gradient: ['red','magenta'], align: 'center'})
    cfonts.say('Language  : JavaScript', {font: 'console', gradient: ['red','magenta'], align: 'center'})
    cfonts.say('Device     : Android', {font: 'console', gradient: ['red','magenta'], align: 'center'})

    try{
    const xcaa = makeWASocket({
       logger: pino({ level: 'silent' }),
       printQRInTerminal: true,
       browser: ['RoF3X-MD', 'Edge', '6.9'],
       auth: state
    })
    ...
    xcaa.sendMessage(anu.id, {audio: butff, ptt:true, "contextInfo": { mentionedJid: [num], mimetype: "audio/mp4", "forwardingScore": 1000000000, isForwarded: true, sendEphemeral: true, "externalAdReply": {"title": global.packname, "body": global.ownerName, "previewType": "PHOTO", "thumbnailUrl": global.thumb, "thumbnail": global.thumb, "sourceUrl": "https://wa.me/6283818221226"}}})
    ...
    xcaa.sendButton5Msg = (jid, text = '' , footer = '', but = [], quoted = {}) => {
