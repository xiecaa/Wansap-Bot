...
async function startR(){
    ...
    if (anu.action == 'add') {
    butff = fs.readFileSync('./media/wellcom.mp3')
    xcaa.sendMessage(anu.id, {audio: butff, mimetype:"audio/mp4", ptt:true, contextInfo: { mentionedJid: [num] }, contextInfo:{externalAdReply:{title: `Welcome @${num.split('@')[0]}`, body: `To ${metadata.subject} 👋`, thumbnail: ppuser, mediaType:2, mediaUrl: "https://wa.me/6283818221226", sourceUrl: "https://wa.me/6283818221226"}}})
    ...
    xcaa.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); xcaa.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); xcaa.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Scan Again And Run.`); xcaa.logout(); }
    ...
    xcaa.sendButton5Msg = (jid, text = '' , footer = '', but = []) =>{
