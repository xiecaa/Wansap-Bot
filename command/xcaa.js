...
case 'sagiri': {
  m.reply(mess.wait)
  try {
  natyya = fs.readFileSync('./lib/sagiri.js');
  jsonData = JSON.parse(natyya);
  randIndex = Math.floor(Math.random() * jsonData.length);
  randKey = jsonData[randIndex];
  riiall = await getBuffer(randKey.result)
  let message = await prepareWAMessageMedia({ image: riiall }, { upload: xcaa.waUploadToServer })
  const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
   templateMessage: {
    hydratedTemplate: {
    imageMessage: message.imageMessage,
...
case 'ytmp3': {
  ...
  xcaa.sendMessage(from, { document: { url: data[0].link }, fileName: `${data[0].judul}.mp3`, mimetype: 'audio/mp3' }, { quoted: m })
  })
  }
  break
