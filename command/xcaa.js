...
case 'getcase': case 'getcs': {
  ...
  break
case 'rest': case 'reset': case 'resetb': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
...
case 'aesthetic': {
  m.reply(mess.wait)
  let tek = fs.readFileSync('./lib/aesthetic.json')
  result = tek[Math.floor(Math.random() * tek.length)]
  let buttonsMess = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Next Image'}, type: 1}]
  xcaa.sendMessage(m.chat, { image: { url: result }, caption: global.mess.sukses, buttons: buttonsMess }, { quoted: m })
  }
  break
case 'darkjoke': case 'meme': case 'darkjokes': {
  m.reply(mess.wait)
  let joke = fs.readFileSync('./lib/darkjokes.json')
  result = joke[Math.floor(Math.random() * joke.length)]
  let buttonsMess = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Next Image'}, type: 1}]
  xcaa.sendMessage(m.chat, { image: { url: result }, caption: global.mess.sukses, buttons: buttonsMess }, { quoted: m })
  }
  break
