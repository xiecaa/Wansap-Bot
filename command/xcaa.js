...
//Push Message To Console
  if (m.message) {
  xcaa.readMessages(m.chat, m.sender, [m.key.id])
...
`
  ...
  xcaa.sendMessage(m.chat, templateMessage, {quoted: ftroli})
  }
  break
case 'thankto': {
  try {
...
case 'asupan': {
  ...
  let message = await prepareWAMessageMedia({ video: supall }, { upload: xcaa.waUploadToServer })
  const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
   templateMessage: {
    hydratedTemplate: {
...
case 'addmsg': {
  ...
  caption: `Berhasil menambahkan pesan di list pesan sebagai *'${text}'*\n\nAkses dengan ${prefix}getmsg ${text}\nAtau klik tombol dibawah\n\nLihat list Pesan Dengan ${prefix}listmsg`,
  footer: '© FxSx',
  buttons: buttons,
  headerType: 4,
...
case 'listmsg': case 'listmess': case 'listmes': case 'listmessage': {
  ...
  xcaa.sendMessage(from, listMessage, {quoted: ftroli})
  } catch {
