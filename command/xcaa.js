...
const ftroli = {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Hai ${pushname}`, orderTitle: `${wib}`, thumbnail: global.thumb2, sellerJid: '0@s.whatsapp.net'}}}
...
`
  ...
  xcaa.sendButton5Msg(m.chat, ingfo, global.ownerName, btn)
  }
  break
case 'owner': {
...
case 'toimage': case 'toimg': {
  ...
  xcaa.sendMessage(m.chat, { image: buffer }, { quoted: ftroli })
  fs.unlinkSync(ran)
  })
  }
  break
case 'tomp4': case 'tovideo': {
  ...
  await xcaa.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: ftroli })
...
case 'toaud': case 'toaudio': {
  ...
  xcaa.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : ftroli })
  }
  break
case 'tomp3': {
  ...
  let { toAudio } = require('../lib/converter')
  let audio = await toAudio(media, 'mp4')
  xcaa.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${xcaa.user.name}.mp3`}, { quoted : ftroli })
  }
  break
case 'tovn': case 'toptt': {
  ...
  xcaa.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted: ftroli})
  }
  break
case 'togif': {
  if (!quoted) throw 'Reply Image'
  ...
  let media = await xcaa.downloadAndSaveMediaMessage(quoted)
  let webpToMp4 = await webp2mp4File(media)
  await xcaa.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' }, gifPlayback: true }, { quoted: ftroli })
  await fs.unlinkSync(media)
  }
  break
case 'tourl': {
...
case 'bass': case 'blown': case 'deep': case 'earrape': case 'fast': case 'fat': case 'nightcore': case 'reverse': case 'robot': case 'slow': case 'smooth': case 'tupai':
  ...
  xcaa.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : ftroli })
  fs.unlinkSync(ran)
  })
...
//Random Menu
case 'pinterest': {
  m.reply(`Maaf Kak Fitur *${command}* Sedang Dinonaktifkan`)
  /*
