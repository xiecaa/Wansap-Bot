...
//Push Message To Console
  ...
  if (command) {
  await xcaa.sendPresenceUpdate('composing', m.chat)
  }
...
case 'delete': case 'del': case 'd': {
  ...
  break
case 'scb': {
  m.reply('Maaf Kak Script Bot Ini Masih Privat\nMungkin Nanti Saya Akan Share Script Nya')
  }
  break
...
case 'bcgc': case 'bcgroup': {
  ...
  let tksbc = `「 *BROADCAST* 」\n\n${text}`
  let btnn = [
     {callButton: {displayText: 'Number FxSx', phoneNumber: '+6283815956151'}},
     {quickReplyButton: {displayText: 'Status Bot', id: 'ping'}},
     {quickReplyButton: {displayText: 'Owner Bot', id: 'owner'}},
     {quickReplyButton: {displayText: 'Thanks To', id: 'thankto'}},
     {quickReplyButton: {displayText: 'Script Bot', id: 'scb'}}
  ]
  let templateMessage = {
  document: global.thumb,
  fileName: global.packname,
...
case 'aesthetic': {
  m.reply(mess.wait)
  let data = fs.readFileSync('./lib/aesthetic.js');
  jsonData = JSON.parse(data);
  randXcaa = Math.floor(Math.random() * jsonData.length);
  randKey = jsonData[randXcaa];
  tektik = await getBuffer(randKey.result)
  let buttonsMess = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Next Image'}, type: 1}]
  xcaa.sendMessage(m.chat, { image: { url: tektik }, caption: global.mess.sukses, buttons: buttonsMess }, { quoted: m })
  }
  break
case 'darkjoke': case 'meme': case 'darkjokes': {
  m.reply(mess.wait)
  let data = fs.readFileSync('./lib/darkjokes.js');
  jsonData = JSON.parse(data);
  randXcaa = Math.floor(Math.random() * jsonData.length);
  randKey = jsonData[randXcaa];
  darkjok = await getBuffer(randKey.result)
  let buttonsMess = [{buttonId: `${prefix + command}`, buttonText: {displayText: 'Next Image'}, type: 1}]
  xcaa.sendMessage(m.chat, { image: { url: darkjok }, caption: global.mess.sukses, buttons: buttonsMess }, { quoted: m })
  }
  break
//Downloader
  ...
  const template = generateWAMessageFromContent(yoy, proto.Message.fromObject({ 
   templateMessage: { 
    hydratedTemplate: { 
     ...
     locationMessage: {
     jpegThumbnail: anu.thumbnail }, 
     hydratedFooterText: global.ownerName, 
     hydratedButtons: [{
     urlButton: {
       displayText: 'Url Play',
       url: `${anu.url}`
     }
  }, {
     quickReplyButton: {
...
//Islamic
  ...
  break
case 'juzamma': {
  m.reply('Maaf Fitur *Juzamma* Sedang Dinonaktifkan*')
  /*if (args[0] === 'pdf') {
  m.reply(mess.wait)
